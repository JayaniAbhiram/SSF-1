...
const DashboardContent = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const adminsResponse = await axios.get('http://localhost:5001/admins/count');
        const donorsResponse = await axios.get('http://localhost:5001/donors/count');
        const volunteersResponse = await axios.get('http://localhost:5001/volunteers/count');
        const unprocessedResponse = await axios.get('http://localhost:5001/donate/count/unprocessed');
        const uncollectedResponse = await axios.get('http://localhost:5001/donate/count/uncollected');
        const collectedResponse = await axios.get('http://localhost:5001/donate/count/collected');
